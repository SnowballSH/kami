...
export class RemoteStick {
  ...
  attach(): Detach {
    ...
    source.addEventListener("error", () => this.letGo());
    return () => {
      source.close();
      this.letGo();
    };
  ...
  private handleMessage(data: unknown): void {
    ...
    this.publish(pressedDirections(state));
    this.watchCat(state);
  }

  private watchCat(state: ControllerState): void {
    const held = holdsCat(state);
    if (held && !this.catHeld) this.options.onCat?.();
    this.catHeld = held;
  }
