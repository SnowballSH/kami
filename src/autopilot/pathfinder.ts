...
import { CELL_PX, CellFlag, type CellRange, type Chart, COLS, ROWS } from "./chart";
import type { Objective, Scene } from "./types";

/** Where her feet are on the grid: body spans columns [c0, c0 + width) and rows [r0 - height, r0). */
...
const key = (node: Node): number => node.r0 * COLS + node.c0;
...
export class Pathfinder {
  ...
  constructor(
    ...
    this.killRow = Math.floor(scene.level.killY / CELL_PX);
  }

  isFree(node: Node): boolean {
    return !this.chart.anyIn(bodyRange(node, this.footprint), CellFlag.solid);
  }
  ...
  isStance(node: Node): boolean {
    if (node.r0 > this.killRow || node.r0 < 0) return false;
  ...
  satisfies(node: Node, goal: Goal): boolean {
    ...
    switch (goal.objective) {
      case "key": {
        if (level.key === undefined) return false;
        const reach = expandRect(body, KEY_PICKUP.reachRatio * body.height);
        return distanceToRect(level.key, reach) <= KEY_PICKUP.radius;
      }
      case "door":
        return level.door !== undefined && rectsOverlap(expandRect(body, CELL_PX), level.door);
      case "exit":
        return rectsOverlap(body, level.exit);
    }
  }
  ...
  /** Where a fall down column `c0` ends: on footing, on a hold, or inside the goal (the exit lies in a ditch). */
  private landing(c0: number, fromRow: number): Node | null {
    for (let r0 = fromRow + 1; r0 <= this.killRow && r0 < ROWS + this.footprint.rows; r0++) {
      const node = { c0, r0 };
  ...
  private *bounces(node: Node): Generator<Edge> {
    ...
    const riseRows = Math.min(ROWS, Math.floor(arc.apexPx / CELL_PX));
    for (let r0 = node.r0 - riseRows; r0 < node.r0 - BOUNCE_TOUCH_ROWS; r0++) {
      ...
      for (let c0 = node.c0 - driftCols; c0 <= node.c0 + driftCols; c0++) {
        const to = { c0, r0 };
        if (c0 < 0 || c0 + cols > COLS || !this.isLandable(to) || !this.isFree(to)) continue;
