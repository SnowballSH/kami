...
| `modes/` | `GameMode`: what the player is when a room opens, win/loss, which laws and natures the page takes, room or endless page, help offered or on request, shared or alone; `EMBODIED_MODE` is today's play, `PUZZLE_MODE` seven staged rooms with the Sumikui loose (`modes/puzzle/`, `docs/puzzles.md`), `SANDBOX_MODE` the endless shared page, `SPIRIT_MODE` a contract (`docs/modes.md`) | `createDirector`, `modeFor`, `allowsLaw`, `refusalLine`, `PUZZLE_ROOMS` |
...
- **Mode.** `GameModules.mode` (default `EMBODIED_MODE`; `?mode=puzzle` and `?mode=sandbox` in the URL pick `PUZZLE_MODE` and `SANDBOX_MODE` through `modeInUrl`) picks a `ModeDirector` that is asked on room open and whether a sim event won the room (its `witness`/`named` seams for changing bodies are declared, not yet called). Laws the mode forbids stay plain writing with Kami's refusal beneath (`refusalLine`: the mode's own line for that dial, else the stock one); `autopilot: "forbidden"` keeps her from walking herself; `page: "endless"` reads the board id as `endlessBoard(id)`; `help: "on-request"` turns the stuck detector off and routes *help* / *give me an idea* through `counsel/`; `sharing: "live"` follows the page (below). Any mode but the embodied one shows its `card` as a title card on open, unless the room it opens is staged with a card of its own. See `docs/modes.md`.
