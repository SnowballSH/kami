...
| `modes/` | `GameMode`: what the player is when a room opens, win/loss, which laws and natures the page takes, room or endless page, help offered or on request, shared or alone; `EMBODIED_MODE` is today's play, `SANDBOX_MODE` the endless shared page, `PUZZLE_MODE` three staged rooms with the Sumikui loose (`modes/puzzle/`, `docs/puzzles.md`), `SPIRIT_MODE` opens as a soul, `BOSS_MODE` is the two-player fight (`docs/modes.md`, `docs/boss.md`); `SpiritDirector` incarnates a drawing named as a body (`bodyNames.ts`) | `createDirector`, `modeFor`, `allowsLaw`, `refusalLine`, `PUZZLE_ROOMS`, `namesABody` |
...
| `sync/` | The wire for shared pages (`BoardChange`, `FeedMessage`, `FeedCursor` and its `<boot>:<seq>` form, zod mirrors shared with the server) and `BoardLink`: follows a board's event stream from the cursor its snapshot was read at, follows again when the server closes it, passes over the echoes of this device's own writes (`LocalEdits`, fed by `EditTrackingStore`), hands changes and presence to the game, reports Alice's position (a ghost always wears Kami's sketch, `ghostOf`) | `BoardLink`, `EditTrackingStore`, `mintPeerId`, `parseFeedMessage` |
| `notes/` | The `Note` type | — |
...
- **The paper's turn** (`paper.ts`). `PaperTurn` holds the angle the page is turned on screen: the `tilt` law plus what the `worldSpin` law has accumulated tick by tick (under `timeScale`; a new tilt restarts the count), wrapped to ±180°. Each board's world owns its own `PaperTurn`, so a board (re)opens at the tilt law's angle with no spin carried over from the last one. `Simulation.paperAngle()` exposes it and `Game` hands it to `CameraRig.turnTo` each frame, so the *camera* turns — the engine's gravity and every body stay in paper coordinates and nothing about Alice, creatures, vehicles, portals, the Sumikui or the autopilot changes. Only loose ink (`isLooseInk`: not a walker, hopper, flier or vehicle, not static) feels the room: `tumble` pushes each such body by the difference between the room's down seen on the turned paper and the paper's own gravity, so a pebble slides off a sideways page and falls up off an upside-down one while Alice keeps walking on it. Body-level `spin` (a drawing turning) is unrelated to `worldSpin` (the page turning).
- **The Sumikui, the ink eater** (`sumikui.ts`). Off by default; `inkEater = 1` puts one in the world (`BoardWorld.sumikui`), `0` removes it, so erasing the summoning note or writing a banishment seals it through the same fold as every other law. It is not a Matter body: a point that drifts behind Alice's shoulder, awake from the first tick after the summons (`sumikui-woke` fires once, on that tick — the law *is* the wake). Everything on the paper is ink to it, so each tick it scores a `Quarry` over a `HuntingGround` and returns what it has finished eating: **ink** — every drawing on the page that is `edible`: not a role fixed to the board (`pinned` natures: goals, portals, hazards, spawn, solids) and not a prop of a scene (`InkEntity.provenance === "scenery"`, set by `Game.dress` through `Simulation.addDrawing(drawing, "scenery")` and saved as `StoredDrawing.provenance`, so a reload or another device restores it the same; the player's and Kami's own ink is `"drawn"`, stored without the field), chewed for `mealTimeFor(cost × size)` = `SUMIKUI_BITE_MS + SUMIKUI_CHEW_MS_PER_PX × px`, capped at `SUMIKUI_MEAL_MAX_MS` — a pebble in about 1.5 s, a long bridge in several — and then `inks.remove`d (`devoured`); **paper** — the board's own solid under an Alice's feet, chewed for `SUMIKUI_PAPER_BITE_MS` and then bitten out (`paper-bitten`); **Alice** herself, held for `SUMIKUI_CATCH_MS` and swallowed (`alice-devoured`, which sets `aliceLost` so she respawns through the ordinary `fell` path; it is gorged for 8 s afterwards and its pace starts over). What she depends on outranks clutter (`WORTH`): ink she stands on, then the ground under her, then her when she is within a lunge, then ink beside her, then whatever is nearest to it; a meal between its teeth is kept until it is gone or lost — a twin a `clones` law dismisses is gone from the `HuntingGround`, so it lets go of her and of the paper under her feet. Where Kami sets her down (spawn and checkpoints, `SUMIKUI_HALLOWED_PX`) is hallowed: neither that paper nor an Alice standing on it is prey, and ink fallen off the bottom of the page is lost to it too. It starts at `SUMIKUI_BASE_SPEED` — about half her walking pace, so she can outrun it early — and doubles every 20 s awake, clamped to `SUMIKUI_MAX_SPEED`; it only holds Alice as a meal while it can keep up with her. `BoardProps` owns the bitten paper: each solid is split into the rectangles left around its `Scar`s and rebuilt as static bodies, scars heal after `SUMIKUI_SCAR_HEALS_MS` (`paper-healed`), and `WorldSnapshot.bites` carries the holes to the chart (`Scene.bites`, cleared from the solid cells) and the renderer. `sumikuiPainter.ts` draws it (a breathing blot with one pale eye whose pupil widens with what it hunts, a dripping trail, a halo that darkens with time awake); `inkPainter` dissolves the drawing between its teeth stroke by stroke as `SumikuiSnapshot.bite` climbs, Alice fades as it closes on her, `boardPainter` paints the ragged holes, and `feedingParticles.ts` paints a capped handful of ink flecks flying from the bite frontier (or her feet, or her) to its mouth with a few drips falling, a pure function of the snapshot and `nowMs`, while its phase is `feeding`. `game.ts` recites the summoning lore over three notes, remarks when it wakes, feeds, bites the ground and swallows her, and says the sealing line in place of "struck from the laws" when its law goes. The player's strokes are never touched: presentation only, and only `InkLayer.remove` takes ink out of the world.
...
- **The key belongs to the party.** `BoardProps.keyTaken` is the one truth: once any Alice takes the key, whichever Alice reaches the door opens it. `AliceController.hasKey` only says who is seen holding it (`AliceSnapshot.hasKey`, the key in her hand). Controllers come and go — twins are dismissed, `disembody`/`incarnate` build a fresh one — so whenever the roster changes and nobody holds a taken key, Alice herself takes it (`MatterSimulation.rosterChanged`); the key is never lost with a body.
...
Alice's snapshot carries the law's desired `sizeMultiplier`, her `innateScale` (how much taller than Kami's Alice a drawn body was drawn), her current `scale` and the granted `headingScale`. `footprintFor` scales her current bounds by a target scale over her current `scale`, exactly as the sim's growth clearance does, and takes the target from `aliceScaleFor` — the same function behind `AliceController.scaleFor` — so a drawn body is planned at its own height and proportions. Current planning footprints cover both the live body and its active resize target, allowing Alice to leave a low ceiling while law growth is deferred; hypothetical meals use the size the sim would grow or shrink her to. Plans expire when the multiplier or footprint changes. Growth checks the full target body against fixed solids, excluding the meal being consumed, for Alice and her twins.
...
- **The Sumikui** (`dread.ts`). While it is hunting or feeding on her, on the paper under her, or on the drawing she stands on, within `DREAD_PX`, the errand becomes `flee`: `Pathfinder.awayFrom` finds the cheapest stance at least `SAFE_PX` (past where it gives up the chase) from it, or failing that the reachable stance farthest from it, flagged unsafe so `status.stuck` reports her cornered (*"Nowhere left to run. Draw her a way out, quickly."*). Fleeing replans every 10 ticks, never sulks, and ends the moment it loses interest (sated, or after other ink). A Sumikui merely prowling, or chewing ink she does not stand on, does not distract her; one chewing the very ink under her feet (`worldBoundsOf` the drawing it is `chewing`, against her feet) is a threat like any other. Ordinary routes are planned on the board `afterTheMeal` — without the drawing it is chewing — so she never sets out over a dissolving bridge; already standing on one, she races across while it stands.
- **Replanning.** Every ¼ s, plus immediately on `invalidate()` — `game/` calls it whenever a drawing commits, is named, ruled, erased or eaten, a rule is enacted or repealed, or a board opens — and whenever her size or key/door progress changes. A route that stops making progress for four seconds is dropped and she sulks briefly before trying again. Routine re-plans (the ¼ s and 2 s clocks) ask the pilot's `Cadence`; a `Party` shares one `OnePerTick` among its pilots (`autopilot/cadence.ts`), so at most one of them re-reads the board on a routine tick and the rest go a tick or two later — and, since each clock restarts at its own re-plan, stay spread. Urgent causes (stale, a changed scene, fleeing) never wait. A pilot on her own is `UNHURRIED`.
...
Canvas 2D at device pixel ratio (cap 2). `toWorld(client, camera)` and `viewport()` are the only geometry it exports. Per frame: clear to white → faint dot grid that thins out as you zoom away → the board's pre-sketched solids (roughjs, seeded, cached per board as drawables and replayed under the camera transform; glass pale blue; no-ink zones red hatching; goal a scribbled black hole; door; key) → inks (perfect-freehand at half the physical ink thickness, thinning by the pen's own pressure when the pointer reported it and by speed otherwise; black until awake, then the nature's marker tint, ~500 ms shiver) → notes (`handwriting.reveal(script, now - writtenAtMs)` through perfect-freehand, thinner than ink; blue for Kami, black for the player, green/red by tone; tappable ones underlined; `opacity`) → ghosts (other devices' Alices on a shared page, faint) → the tear → her twins, then Alice (black marker stick doodle, two-frame walk, scales with size, holds the key; drawn from an `AliceFigure` that `render/animation/aliceAnimator.ts` composes per Alice from her snapshot deltas and `frame.events` — hop into a seat, landing squash, flip on turning, swell on growing, portal pop, ink-drip and re-ink after a fall — presentation only, the sim is never touched; each twin wears a tinted, numbered ribbon — `alicePainter.ts:AliceBadge`, colour by index — and the selected Alice a small caret over her head, `frame.selectedAlice`; a Sumikui closing on one of them fades only its prey, `SumikuiSnapshot.prey`; a drawn body (`AliceSnapshot.look`) is her own strokes filled with the pose instead of the figure, fresh grafts haloed blue) or, when nobody is on the board, the soul (a pulsing blue heart) → the tear's snippers, their telegraphs and the cut marks (`bossPainter.ts`) → held inks (settled strokes still being read, fading once they were words) → active strokes (red when the verdict isn't `ok`) → night (`NightPainter`: when `frame.daylight < 1`, a dark layer on an offscreen canvas with radial pools cut out around Alice and every `lantern` ink, composited over the board) → the ink health bar and, for a headless body, the dim veil. Cull anything whose bounds miss the viewport. Depends on `handwriting/types` only; `createRenderer(canvas, handwriting)`.

**Path caches.** perfect-freehand is the expensive part of a frame, so nothing is traced twice. `inkPath.ts` keeps each stroke's outline per pen in a `WeakMap` keyed by the stroke array (checked against its length and last point, since the pending stroke grows in place under the pen), so a path rebuilt from mostly-known strokes — the drawing being drawn, Kami's ink being inked in, a retrace's added strokes — only traces the stroke that changed. That works because `ink/retrace.ts` hands back the same stroke arrays while they are not moving: `drawnIn` returns a finished stroke itself, and a fully tidied stroke is memoised against its goal. `InkPainter` keeps one `Path2D` per drawing keyed by `drawing.strokes`, built only once the ink is in view; the renderer calls `retain(ids)` once per frame with every ink on the board to forget erased ones, and `paintOver` repaints a ridden vehicle over its rider without touching the cache. A drawn body's path is kept per `body.strokes` array.
...
under the top-left cluster, so it stays in view while Sandbox's share button stands in for the board
...
- **Camera** follows the selected Alice loosely when she walks outside a central dead-zone, leaning `COMPANY_LEAN` of the way toward any other Alice within half a screen so close company stays in frame; any manual pan or zoom suspends following until she walks again or ⌖ is pressed. Zoom 0.25–4. Its angle is the simulation's paper angle (tilt and world-spin laws); pans are taken along the screen, so dragging still moves the page the way the finger goes. Under `prefers-reduced-motion` (`motionAllowed()`), `SteppedTurn` shows a turning page only in jumps of at least `TURN_STEP_DEGREES` (15°) and settles on the exact angle once the page has held still for `TURN_SETTLE_MS`, so "the world spins slowly" no longer rotates the whole screen every frame while a resting tilt, however small, is still shown as it is. Only the camera steps; the paper, its physics and input mapping (through the same camera) are untouched.
...
- **Staged rooms.** A director may answer `open(board)` with a `RoomStaging`: the room's own world (laws fold over it instead of Earth — `resolvePhysics(rules, base)`), a law policy narrower than the mode's, a title card with a progress mark (`ui/roomCard.ts`), a closing line and the next board. The game shows the card on open, writes the closing line on a win and opens the next room after `NEXT_ROOM_DELAY_MS`. Puzzle mode stages every room with `inkEater: 1` and plays through a `ForgetfulBoardStore`, so nothing a player draws in a room is saved (and the save status says *Not saved*).
- **Endless pages.** An endless board has `killY` of infinity and one strip of ground. `LastFooting` (`sim/footing.ts`) remembers where an Alice last stood; `FALL_LIMIT` below that is `fell`, and she is set down on that footing once — if it is gone and she falls again, on the spawn. `Footings` keeps one per Alice by roster index, so each is judged and set down by where she herself last stood: a twin walking down a staircase of ink is not "fallen" because Alice stayed at the top. Alice herself keeps hers through new bodies; a new twin's starts as a copy of Alice's (she appears beside her), and a dismissed twin's is forgotten when the roster changes. Ink and Alices further than `LOST_DISTANCE` from every solid are lost as in a room, so a page keeps only what someone can stand on or reach.
- **Shared pages** (`sync/boardLink.ts`). With `sharing: "live"` the game loads the board first and then `follow`s it from the `cursor` the load carried (`BoardSnapshot.cursor`, `{ boot, seq }`): `BoardLink` opens `GET /api/boards/:board/events?peer=<id>&since=<boot>:<seq>` (an `EventSource`; the browser sends `Last-Event-ID` on reconnect), so whatever changed between the read and the subscription is replayed, never lost or undone by the older snapshot. The link keeps the last `seq` it handed on and drops anything numbered at or before it. A dropped connection the browser retries by itself (with `Last-Event-ID`); a stream it gives up on — an error status such as a 502 or 401 leaves an `EventSource` `CLOSED` for good — the link follows again from its last `seq` after `RECONNECT_BACKOFF_MS` (1 s, doubling to 30 s, reset by any message), or reloads the page when it never learned a cursor. Every `put` / `delete` / `clear` that arrives — another device's or this one's echoed back — lands through the same seams a load does: `placeDrawing` (add, or retrace changed strokes and re-rule), `placeNote`, `placeLaw` (the rule book `place`s it in chronological order and refolds; Kami's gloss is rewritten), `dropDrawing`, `dropNote`, `dropLaw` (repeal), and `clear` empties the page (`open(…, { blank: true })`) and goes on following the same stream, so the other device's first strokes after its clear are not missed. Echoes of this device's own writes are passed over before they reach the game (`sync/localEdits.ts`): the game's store is wrapped in an `EditTrackingStore` that tells the link each write before it is made, and until the echo of its latest write to an entity arrives, whatever else arrives about that entity is older than what the board shows — a drawing erased here does not come back on the late echo of its drawing, and the late echo of a clear does not wipe what was drawn since. The server orders writes, so anything relayed after that echo lands after this device's write on the server too, and applies. A peer's clear supersedes every pending expectation. The expectations start afresh with each `follow` (every follow is from a fresh snapshot, which already holds this device's unsaved writes). A change identical to what is already on the board is ignored as well. A `resync` (the server's log does not reach back to our cursor, or the cursor is from before a server restart) stops the stream; the board is loaded again and folded into the page as it stands (`Game.catchUp`/`reconcile`: drawings, laws and stored notes the server no longer holds are dropped, the rest placed) rather than reopened, so Alice, the stroke under the pen and the undo history survive a server restart; then it is followed from the new cursor. Every `PRESENCE_INTERVAL_MS` the link `POST`s Alice's snapshot to `/api/boards/:board/presence`; other peers' snapshots arrive as `presence` messages and are painted as **ghosts** (`GHOST_ALPHA`, culled offscreen, never in the sim: a ghost is a picture of someone else's Alice, not a body). The peer id (`mintPeerId`) is a name for the tab, not an account. The HUD swaps the board menu for the share panel — board id, `?board=<id>&mode=sandbox` link, its QR (`qrcode`), and how many others are on the page.
...
old ones as a `retrace` `InkMotion` (`ink/retrace.ts`; an arrival still inking in is kept instead); for `RETRACE_MS` `views()` shows `retracedStrokes(from, to, progress)`, and says where it is settling (`InkView.settling`) — the ink
...
strokes and scripts once, poses every frame; ink still being inked in or tidied goes once as its
final strokes and its `InkMotion`, which the screen plays on the frames' clock) and sent while the
server says a screen is watching.
