...
The cabinet firmware's `S,dir,ink,cat,px,py` frames use a **serial-only adapter** into the same hub
as controller `arcade`. Direction bits map to movement; INK/CAT are unbound `b`/`x` buttons and
pot readings are validated but not relayed. [R19 / #41](https://github.com/SnowballSH/kami/pull/41)
includes firmware debounce/framing checks and pinned compilation. Knob drawing, INK gestures,
physical CAT-to-speech binding, game-driven LED feedback and a visible reconnect UI remain deferred.
The browser uses SSE, not Web Serial. [hardware.md](hardware.md) documents the supported path,
proposed full panel and outstanding physical checks. Firmware compilation is not physical acceptance.
