...
| `sync/` | The wire for shared pages (`BoardChange`, `FeedMessage`, zod mirrors shared with the server) and `BoardLink`: follows a board's event stream, hands changes and presence to the game, reports Alice's position (a ghost always wears Kami's sketch, `ghostOf`) | `BoardLink`, `mintPeerId`, `parseFeedMessage` |
| `notes/` | The `Note` type | — |
