...
| `modes/` | `GameMode`: what the player is when a room opens, win/loss, which laws and natures the page takes; `EMBODIED_MODE` is today's play, `SPIRIT_MODE` opens as a soul, `BOSS_MODE` is the two-player fight (`docs/modes.md`, `docs/boss.md`); `SpiritDirector` incarnates a drawing named as a body (`bodyNames.ts`) | `createDirector`, `modeFor`, `allowsLaw`, `namesABody` |
| `notes/` | The `Note` type | — |
