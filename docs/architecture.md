...
| `modes/` | `GameMode`: what the player is when a room opens, win/loss, which laws and natures the page takes, room or endless page, help offered or on request, shared or alone; `EMBODIED_MODE` is today's play, `SANDBOX_MODE` the endless shared page, `SPIRIT_MODE` a contract (`docs/modes.md`) | `createDirector`, `modeFor`, `allowsLaw`, `refusalLine` |
