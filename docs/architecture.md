...
| `modes/` | `GameMode`: what the player is when a room opens, win/loss, which laws and natures the page takes; `EMBODIED_MODE` is today's play, `PUZZLE_MODE` seven staged rooms with the Sumikui loose (`modes/puzzle/`, `docs/puzzles.md`), `SPIRIT_MODE` a contract (`docs/modes.md`) | `createDirector`, `modeFor`, `allowsLaw`, `PUZZLE_ROOMS` |
| `notes/` | The `Note` type | — |
