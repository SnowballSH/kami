...
Alice's snapshot carries the effective `sizeMultiplier`. Current planning footprints cover both the live body and its resize target; hypothetical meals use the same law-scaled dimensions as simulation growth clearance. Plans expire when the multiplier or footprint changes. Growth checks the full target body against fixed solids, excluding the meal being consumed.
