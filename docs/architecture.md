...
- **Mode.** `GameModules.mode` (default `EMBODIED_MODE`; `?mode=` in the URL through `modeInUrl`) picks a `ModeDirector` that is asked on room open, on every naming (`named`) and every sim event (`witness`, `won`); the game enacts the `EmbodimentTransition`s it answers — `sim.disembody` / `incarnate` / `openTear`, and the mode's `LossRule` when she is unmade (`board-restarts` reopens the room after `RESTART_AFTER_MS`). While the player is a soul `sim.alices()` is empty, so the `Party` drives nobody, the camera holds on the soul, and nothing is announced to a shared page or read for counsel; a drawn body is Alice herself's alone (twins wear Kami's sketch) and the servant hunts her, not her twins. In a spirit-opening mode ink may touch her and a committed drawing that reaches her body is a graft, not a drawing. Laws the mode forbids stay plain writing with Kami's refusal beneath (`refusalLine`: the mode's own line for that dial, else the stock one); `autopilot: "forbidden"` keeps her from walking herself; `page: "endless"` reads the board id as `endlessBoard(id)`; `help: "on-request"` turns the stuck detector off and routes *help* / *give me an idea* through `counsel/`; `sharing: "live"` follows the page (below). Any mode but the embodied one shows its `card` as a title card on open, with its `roles` lines under the wordmark. See `docs/modes.md`, `docs/boss.md`.
