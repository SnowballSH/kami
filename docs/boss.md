...
**Snipping** (`snip`): a cut is a segment; every stroke it crosses is removed whole. The heart is cut only when it is *bare* — no torso stroke left around it — and the segment passes within `heartRadius` of it; that is the loss. A cut through a heart still wrapped in a torso takes the torso, and the mercy window that follows is the drawer's chance to wrap it again.
