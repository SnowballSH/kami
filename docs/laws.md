...
`recognisers/motion.ts` does the same for body effects, plus the target: `normalise` keeps the nouns that followed a determiner (“the *wheel*”, “every *rock*”), and the first of those that is neither a dial word nor a number, together with the unknown words right after it, is the `named` target (“the *lily pads* drift left” names `lily pads`, “the *big rock* is heavy” names `big rock`); “everything” / “all drawings” is `all`; a sentence about Alice is passed on so her dials keep their own grammar. The world dial recognisers run first, so “everything is bouncy” stays the world's `bounciness` and only sentences the world has no dial for (spin, thrust, mass, grip) fall through to `all`.

**Drifting is thrust with a heading.** Besides its own words (“accelerates”, “propelled”, “zooms”), thrust answers to words of travel — drift, sail, roll, move, go, travel, head, glide, slide, wander, drive, rise, sink, fall — but only when the sentence says which way: “the boat sails to the right”, “the cart moves left”, “the balloon rises” (*rise* heads up, *sink* and *fall* head down). Without a heading those words are not a law about thrust: “the cart moves” is a remark, “the dog moves faster” is pace, “the bird glides” is wings. “Slowly”/“gently” and “fast” give the gentle and hard thrusts as for “accelerates”. A drawing told to *stop* with no other dial in the sentence (“the cart stops”) has its thrust turned off; “the wheel stops spinning” is still spin.
...
**A pronoun stands for the drawing it was written beside.** Players write next to what they drew: “it drifts to the right”, “make it bouncy”, “they float”. Compiling is therefore `compile : Text × Maybe Context → Maybe (Edit × Gloss)`, where `Context = { referent: word }` is supplied by `game/` from the named drawing nearest the note (`drawingNear`, within naming reach): the **head noun** of its name (`rules/referents.ts: referentOf` — the last word before any qualifier, articles dropped: “a little red boat” → `boat`, “a cat that follows her” → `cat`). `normalise` resolves the sentence's pronoun (*it, its, this, that, this/that one, they, them, these, those…*) to that word — only when the pronoun comes before any mention of Alice, so “it follows me” is about the drawing and “Alice chases it” is not — and `targetOf` falls back to it after an explicit subject and after `all`: “the rock spins” and “everything spins” still mean what they say, and world laws (“it is cold”) are recognised before any body law is tried. A sentence that says the referent's noun itself (“it is a heavy boat”) is a name, not a law. The binding produces an ordinary `named(referent)`:

- *Why `named` and not a new `Target` kind for one drawing.* A law about a drawing is a law about its kind in Kami (“the boat sails” moves every boat); a pronoun is only another way to say the noun, so it binds to the same target the player could have written, the fold and `speaksOf` are unchanged, a rule stored today reads the same on any client, and `RuleEffect` — the client–server seam — does not change. A per-drawing target would need a drawing id in persisted rules, a schema and prompt change, and would still leave a nameless drawing with nothing to be.
- *A nameless drawing* has no referent. If the sentence would be a body law once bound (`speaksOfReferent`), Kami answers *“What is it, though? Name it, and it will listen.”* under the note instead of naming the drawing with the sentence or asking the model.
- *The model* gets the same context: `POST /api/compile` takes an optional `referent` (one lowercase word) and the server appends `Beside: <noun>` to the note; the system prompt says a pronoun means `named(<noun>)`.
...
| weather | `temperature` | `slippery` melts above 30 °C, `floaty` burns off above 60 °C, after a dwell; emits `perished`. Warm air lifts: a `floaty` drawing's rise is multiplied by `floatLiftAt(T) = clamp((T + 10) / 30, −0.5, 1.5)` — exactly 1 at Earth's 20 °C, faster in the heat until it burns off, hovering at −10 °C and sinking gently below (so balloons sink on Mars); read through `NatureWorld.temperature`. Puzzle rooms never take `temperature`, so their floaty drawings always rise as on Earth |
...
| `the lily pads drift left` · `the balloon rises` | `set(thrust, (−0.5, 0)) of named(lily pads)` · `set(thrust, (0, −0.5)) of named(balloon)` | `W.bodies ++ [...]` | motion: every “lily pad” drawing pushes itself leftward, the balloon upward |
| `the rock is heavy` then `the rock is not heavy` | `set(mass, 2) of named(rock)`, `set(mass, 1) of named(rock)` | the later wins: the rock weighs as it was drawn | motion |
