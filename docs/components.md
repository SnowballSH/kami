...
| Placement | `src/ink/placement.ts`, `src/board/ground.ts` | built | `groundSolids` derives board floors; points strictly under the applicable ground are quietly refused as `under-ground`, while ditches and overhead scenery remain drawable |
...
| Portals | `src/sim/portals.ts` | built | drawings named "portal" link in drawing order; exit refuses re-entry until she leaves it; presentation shrinks/spins Alice into the entry ring, then grows her out of the exit with a ring pulse |
...
| HUD | `src/ui/hud.ts`, `toolbar.ts`, `lawsPanel.ts`, `controls.ts`, `persistenceStatus.ts`, `accessGate.ts`, `titleCard.ts`, `roomCard.ts` | built | always-available clear-page button behind a second tap; at phone width (≤ 640 px) the toolbar drops to a second row, the tidy slider sits above the view island and the laws panel below the toolbar; labeled, focusable 44px controls; laws panel stays visible with an empty-state hint; HUD dragging cannot select text; accessible title/room dialogs dismiss on tap or Enter/Space and honor reduced motion; what Kami writes is echoed to screen readers through a hidden polite status region (`spokenLines.ts`); autopilot switch; save status as its own line under the top-left cluster (*Not saved* when the store keeps nothing); sign-out beside home on a shared server; access gate hands the game a `Connection`, and offline play hides Share |
...
| Notes | `src/notes/types.ts`, `src/game/noteBook.ts`, `noteLayout.ts` | built | player and Kami notes; laid out to avoid overlap, kept above ground, and clamped inside the visible world. All hurried note fades share `NOTE_FADE_MS` (700 ms). A note may own a rule. Nothing written stays: Kami's replies fade in 6–12 s, the player's words and Kami's labels 12 s after they are answered (`NoteBook.release`), restored notes 12 s after the board opens; only the wordmark is permanent. A saved note is restored exactly where it was written, never clamped into this device's view. Only the device that answers for a note (`Game.ownNotes`: it wrote it, or restored it with nobody else to answer for it) deletes it from the page when it fades; on a shared page another device's note stays until its writer lets it go (or leaves this screen after 2 minutes), and a restored note is tidied away only once it is 10 minutes old Fading never repeals — laws live on in the laws panel, drawings keep their names |
...
| Sandbox | `src/modes/sandboxMode.ts`, `src/board/boards/endless.ts`, `src/sim/footing.ts`, `src/autopilot/chart.ts` (`WINDOW_PX`), `src/autopilot/pilot.ts` (`explore`), `src/counsel/*`, `src/sync/*`, `src/ui/sharePanel.ts`, `src/ui/titleCard.ts`, `src/game/launch.ts` | built | `?mode=sandbox`: an endless shared page (`?board=<id>`, default `sandbox`); the chart is windowed around every Alice and the pilot explores toward the newest ink; Kami helps only when asked ("help", "give me an idea": a bridge over a gap, a ladder up a wall, an idea on open page); `inkEater` refused with "Nothing hungry lives on this page."; accessible, dismissible mode title card and share panel (link + QR + who is here); see [modes.md](modes.md) |
| Puzzle | `src/modes/puzzle/{mode,rooms,puzzleDirector}.ts`, `src/modes/director.ts`, `src/board/boards/puzzles/*`, `src/game/forgetfulStore.ts`, `src/ui/roomCard.ts`, `src/sim/nightfall.ts` | built | three staged rooms, one drawn or written idea each, Sumikui loose from the first frame, laws fold over the room's own world, accessible, dismissible room card + progress mark, goal → next room or run complete with Solved/Lost cards, nothing saved (the status says *Not saved*); `menu: run` swaps the board menu for a two-tap *start the run over* beside home; `?mode=puzzle`; see [puzzles.md](puzzles.md) |
