...
- asks `director.witness(event)` for every sim event and `director.named(id, ruling)` for every naming, and enacts the transitions: `incarnated by drawing` → `sim.incarnate(id, name)` (the drawing leaves the ink layer and the store; its strokes are her body), `tear-opens` → `sim.openTear()`, `unmade` → whatever the mode's `LossRule` says (`unmade`: `sim.disembody()`, she is a soul again; `board-restarts`: Kami says why, and the room reopens after a beat under the mode's `again` card; `respawn`: the sim's own checkpoint path);
...
- **An endless page** (`board/boards/endless.ts`). `PageKind` is `"room" | "endless"`. An endless board is a very wide floor `ENDLESS_GROUND` (12,000 px, the spawn in the middle of a 500 px clearing) with `ENDLESS_HIGH_GROUNDS` standing along it for demos — to the right a step to jump, two plateaus with a gap to bridge, three tall bars and a shelf in the air; to the left a staircase of ledges, a tower and three columns — no zones, no goal, no rabbit hole and `killY` of infinity: there is no edge to fall off. Beyond the floor there is nothing until someone draws. (`endlessPage(id, solids)` makes a page of any other paper; the tests that walk her off the edge use the short `ENDLESS_STRIP`.) The camera follows her to any coordinate (`cameraRig.test.ts` walks her to ±5,000,000). Falling `FALL_LIMIT` below the last place she stood (each twin counts from her own) is falling off the page; `LastFooting` (`sim/footing.ts`) puts her back on the ink she last stood on, once — if that ink is gone too, the second fall returns her to the spawn.
