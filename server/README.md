...
| `GET /api/boards/:board/events?peer=<id>[&since=<boot>:<seq>]` | Server-Sent Events: every put/delete/clear on the board as it happens, numbered; who else is on it. See the API contract below. |
...
| `GET /api/boards/:board/events` | `?peer=<id>` (`[a-z0-9-]{1,64}`, this device's name on the board, optional) and `?since=<boot>:<seq>` — the `cursor` a `GET /api/boards/:board` answered, or the last change's event id — or, on a browser's own reconnect, its `Last-Event-ID`, which wins over `since` since it is always newer. A bare `<seq>` (no boot) is still accepted and read as this server life's | `text/event-stream`: `retry: 1000`, then the board's changes as `id: <boot>:<seq>` + `data: {"seq","type":"put","kind":"drawings"\|"notes"\|"rules","id","entity"}`, `{"seq","type":"delete","kind","id"}` or `{"seq","type":"clear"}` — first everything after `since` that the server still holds (the last 2000 per board and at most 8 MiB of them, in memory), else — or when `since` names another `boot`, a cursor from before the server restarted — `data: {"type":"resync","seq","boot"}` meaning *reload the board, then follow from the cursor that load carries*; without `since`, `data: {"type":"cursor","seq","boot"}` says where the feed stands. (`boot` on `cursor` and `resync` is additive.) `data: {"type":"presence","peer","alice"}` for every peer on the board on connect and on every report, with `alice: null` when one leaves (its stream closed). `: keep-alive` every 5 s. Every `PUT`/`DELETE` on the board's entities and `DELETE` of the board is echoed to every stream, the sender's included; the schemas are `src/sync/wire.ts` (`feedMessageSchema`) |
| `POST /api/boards/:board/presence` | `{ peer: string, alice: AliceSnapshot }` — `alice` is the client's own `sim` snapshot of her (`src/sim/types.ts`) with `look: { kind: "alice" }` — a drawn body stays on its own page (`ghostOf`, `src/sync/wire.ts`); `innateScale` and `scale` default to `1` when a client leaves them out | `204`; `400` `{ error }` for a bad peer or snapshot. Relayed as a `presence` message; nothing is stored |
...
The board feed (`server/sync/`) is in-memory and per process: sequence numbers restart with the server, so every
cursor carries the process's `boot` (a random name minted at start, `BoardFeed.boot`) and one from another life gets
`resync`, even when its number happens to be in range; so does a cursor the log no longer reaches. The client then
reloads through `GET /api/boards/:board`. That read takes the cursor *before* reading the board, so a change landing
in between is replayed rather than lost (at worst applied twice, which is idempotent).
