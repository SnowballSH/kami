...
describe("BoardFeed", () => {
  ...
  it("keeps no more of the log than its byte budget, and resyncs cursors from before it", () => {
    ...
    expect(heard(feed, "a", 0).messages).toEqual([{ type: "resync", seq: changes }]);
    expect(heard(feed, "a", 2).messages).toEqual([{ type: "resync", seq: changes }]);
  ...
  it("forgets an idle page with no listeners or peers, and numbers it on from where it stood", () => {
    ...
    expect(behind.messages).toEqual([{ type: "resync", seq: 4 }]);
    behind.stop();
    stop();
    now += 2 * PAGE_IDLE_MS;
